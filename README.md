...
The search depth parameter passed in each successive iteration into the main search routine represents the maximum search depth of its Floyd-style quiescence search (see Knuth and Moore, An Analysis of Alpha-Beta Pruning, p. 302), in which "violent" moves (i.e. captures, catles, and queen-promotions) decrement the remaining depth less than do "quiet" moves (i.e. non-captures and underpromotions).
