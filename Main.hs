...
populate n po a b (Game l t ca o u m h e ch)  = if n <= 0 then Game l t ca o u m h h Nothing else if legal then Game l t ca o u m h score (Just winner) else Game l t ca o u m h (specials t) Nothing
    where
        ...
        newbies                               = (if t then concat [if all ((/= temp) . fst) l then if snd temp /= 8 then [Game ((temp,'P'):filter (/= i) l) (not t) ca o (2) (0,0) 0.0 0.0 Nothing] else [Game ((temp,'Q'):filter (/= i) l) (not t) ca o (1) (0,0) 0.0 0.0 Nothing, Game ((temp,'R'):filter (/= i) l) (not t) ca o (2) (0,0) 0.0 0.0 Nothing, Game ((temp,'B'):filter (/= i) l) (not t) ca o (2) (0,0) 0.0 0.0 Nothing, Game ((temp,'N'):filter (/= i) l) (not t) ca o (2) (0,0) 0.0 0.0 Nothing] else [] | i <- filter ((=='P') . snd) l, let temp = ((fst . fst) i, (snd . fst) i + 1)] ++ -- white pawn moves one square or promotes
                                                           ...
                                                           concat [if bounds temp && all (\q -> all (\p -> fst p /= q) l) (init . tail . myrange $ (fst i, temp)) && all (\p -> fst p /= temp || (isLower . snd) p) l then [Game ((temp,'R'):(filter (/= i) . filter ((/= temp) . fst) $ l)) (not t) (replace 2 (ca!!2 || temp == (8,8)) . replace 3 (ca!!3 || temp == (1,8)) . replace 0 (fst i==(8,1) || ca!!0) . replace 1 (fst i == (1,1) || ca!!1) $ ca) o (if any ((== temp) . fst) l then (1) else (2)) (0,0) 0.0 0.0 Nothing] else [] | i <- filter ((== 'R') . snd) l, j <- filter (/=(0,0)) (range((-8,0),(8,0))++range((0,-8),(0,8))), let temp = ((fst . fst) i + fst j, (snd . fst) i + snd j)] ++ -- white rook moves or takes
                                                           concat [if bounds temp && all (\q -> all (\p -> fst p /= q) l) (init . tail . myrange $ (fst i, temp)) && all (\p -> fst p /= temp || (isLower . snd) p) l then [Game ((temp,'B'):(filter (/= i) . filter ((/= temp) . fst) $ l)) (not t) (replace 2 (ca!!2 || temp == (8,8)) . replace 3 (ca!!3 || temp == (1,8)) $ ca) o (if any ((== temp) . fst) l then (1) else (2)) (0,0) 0.0 0.0 Nothing] else [] | i <- filter ((=='B') . snd) l, j <- filter (/=(0,0)) ([(k,k) | k <- [-8..8]]++[(k,-k) | k <- [-8..8]]), let temp = ((fst . fst) i + fst j, (snd . fst) i + snd j)] ++ -- white bishop moves or takes
                                                           ...
                                                           concat [if bounds temp && all (\q -> all (\p -> fst p /= q) l) (init . tail . myrange $ (fst i, temp)) && all (\p -> fst p /= temp || (isLower . snd) p) l then [Game ((temp,'Q'):(filter (/= i) . filter ((/= temp) . fst) $ l)) (not t) (replace 2 (ca!!2 || temp == (8,8)) . replace 3 (ca!!3 || temp == (1,8)) $ ca) o (if any ((== temp) . fst) l then (1) else (2)) (0,0) 0.0 0.0 Nothing] else [] | i <- filter ((== 'Q') . snd) l, j <- filter (/=(0,0)) ([(k,k) | k <- [-8..8]]++[(k,-k) | k <- [-8..8]]++(range((-8,0),(8,0))++range((0,-8),(0,8)))), let temp = ((fst . fst) i + fst j, (snd . fst) i + snd j)] ++ -- white queen moves or takes
                                                           ...
                                                           concat [if bounds temp && all (\q -> all (\p -> fst p /= q) l) (init . tail . myrange $ (fst i, temp)) && all (\p -> fst p /= temp || (isUpper . snd) p) l then [Game ((temp,'r'):(filter (/= i) . filter ((/= temp) . fst) $ l)) (not t) (replace 0 (ca!!0 || temp == (8,1)) . replace 1 (ca!!1 || temp == (1,1)) . replace 2 (fst i==(8,8) || ca!!2) . replace 3 (fst i == (1,8) || ca!!3) $ ca) o (if any ((== temp) . fst) l then (1) else (2)) (0,0) 0.0 0.0 Nothing] else [] | i <- filter ((== 'r') . snd) l, j <- filter (/=(0,0)) (range((-8,0),(8,0))++range((0,-8),(0,8))), let temp = ((fst . fst) i + fst j, (snd . fst) i + snd j)] ++ -- black rook moves or takes
                                                           concat [if bounds temp && all (\q -> all (\p -> fst p /= q) l) (init . tail . myrange $ (fst i, temp)) && all (\p -> fst p /= temp || (isUpper . snd) p) l then [Game ((temp,'b'):(filter (/= i) . filter ((/= temp) . fst) $ l)) (not t) (replace 0 (ca!!0 || temp == (8,1)) . replace 1 (ca!!1 || temp == (1,1)) $ ca) o (if any ((== temp) . fst) l then (1) else (2)) (0,0) 0.0 0.0 Nothing] else [] | i <- filter ((== 'b') . snd) l, j <- filter (/=(0,0)) ([(k,k) | k <- [-8..8]]++[(k,-k) | k <- [-8..8]]), let temp = ((fst . fst) i + fst j, (snd . fst) i + snd j)] ++ -- black bishop moves or takes
                                                           ...
                                                           concat [if bounds temp && all (\q -> all (\p -> fst p /= q) l) (init . tail . myrange $ (fst i, temp)) && all (\p -> fst p /= temp || (isUpper . snd) p) l then [Game ((temp,'q'):(filter (/= i) . filter ((/= temp) . fst) $ l)) (not t) (replace 0 (ca!!0 || temp == (8,1)) . replace 1 (ca!!1 || temp == (1,1)) $ ca) o (if any ((== temp) . fst) l then (1) else (2)) (0,0) 0.0 0.0 Nothing] else [] | i <- filter ((== 'q') . snd) l, j <- filter (/=(0,0)) ([(k,k) | k <- [-8..8]]++[(k,-k) | k <- [-8..8]]++(range((-8,0),(8,0))++range((0,-8),(0,8)))), let temp = ((fst . fst) i + fst j, (snd . fst) i + snd j)] ++ -- black queen moves or takes
...
mover fen depth                               = case length diff of 4 -> case head diff of 0 -> (1,5,1,3) -- Q
    ...
    where
        ...
        l2                                    = assocs . boarder . locations . fromJust . children $ game
